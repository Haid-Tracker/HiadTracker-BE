...
<section class="content">
    <div class="container-fluid">
        ...
        <div class="card card-secondary">
            ...
            <form action="{{ route('admin.cycle-record.store') }}" method="POST">
                @csrf
                <div class="card-body">
                    <!-- User Selection -->
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="blood_volume">
                                    <option value="normal" {{ old('blood_volume') == 'normal' ? 'selected' : '' }}>Normal (≤1 pad/hour)</option>
                                    <option value="heavy" {{ old('blood_volume') == 'heavy' ? 'selected' : '' }}>Heavy (>1 pad/hour)</option>
                                </select>
                            </div>
                        </div>
                        <!-- Cycle Regularity -->
                        <div class="col-md-6">
                            <div class="form-group">
                                ...
                                <select class="form-control" name="cycle_regularity">
                                    <option value="1" {{ old('cycle_regularity') == '1' ? 'selected' : '' }}>Regular</option>
                                    <option value="0" {{ old('cycle_regularity') == '0' ? 'selected' : '' }}>Irregular</option>
                                </select>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="d-flex flex-wrap gap-3">
                            @foreach(['happy', 'sad', 'neutral', 'tired'] as $mood)
                                <div class="form-check mr-4">
                                    <input class="form-check-input" type="radio" name="mood" value="{{ $mood }}" id="mood_{{ $mood }}" {{ old('mood') == $mood ? 'checked' : '' }}>
                    ...
                    <div class="form-group">
                        <div class="custom-control custom-checkbox">
                            <input class="custom-control-input" type="checkbox" id="medication" name="medication" value="1" {{ old('medication') ? 'checked' : '' }}>
                    ...
                    <div class="form-group">
                        <label>Notes</label>
                        <textarea class="form-control" name="notes" rows="3" placeholder="Enter notes">{{ old('notes') }}</textarea>
                    </div>
                </div>
